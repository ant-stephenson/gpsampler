...
def import_data(method, job_id, param_idx, with_pre):
    ...
    # get data
    sweep = pd.read_csv(
        expt_path.joinpath(f"output_sweep_{method}_{param_idx}_{job_id}.csv")
    )
    ...
    # neff (written by lrff runs) is excluded from group keys and averaged.
    excl = ["reject", "err"]
    if "neff" in sweep.columns:
        excl.append("neff")
    sweep_grp = sweep.groupby(sweep.columns.difference(excl).tolist())
    ...
    with LaTeX() as ltx:
        ...
        xticks = [
            ...
            for x in np.geomspace(
                df.loc[:, xlabel].min(), df.loc[:, xlabel].max(), num=4
            )
        ]
        ...
        for _l, _ax in ax.axes_dict.items():
            ...
            try:
                _ax.fill_between(
                    range(df.shape[0]),
                    np.tile(chol_max[_l], df.shape[0]),
                    np.tile(chol_min[_l], df.shape[0]),
                    alpha=0.2,
                    color="gray",
                    interpolate=True,
                )
...
# expectation
rescaled_fidel = (
    "$" + fidel_param + "/" + "\\bar{" + f"{fidel_param}" + "}" + "(n)$"
)
...
save_fig(
    path,
    f"logreject-logD_byN_{method}_{param_idx}_{job_id}_rescaled_new",
    suffix="pdf",
    show=True,
    dpi=600,
    overwrite=True,
...
# %% --- Variant (i): rescale lrff x-axis by n_eff^2  ---
# Only runs when method="lrff" and the neff column is present in the CSV.
# Caption note: n_eff = Tr(K K_xi^{-1}) estimated once per (N,l) via a
# 30-probe Hutchinson trace estimator reusing the harness Cholesky.  The
# vertical black line at D/n_eff^2 = 1 is the effective-dimension bound.
# This is NOT a like-for-like comparison with the RFF panel (which uses
# D/(n/noise_var) on its x-axis); see companion headline figure for that.
if method == "lrff" and "neff" in sweep.columns:
    rescaled_neff2 = "$D/n_{\\mathrm{eff}}^2(n)$"
    sweep.loc[:, rescaled_neff2] = (
        sweep.loc[:, fidel_param] / sweep.loc[:, "neff"] ** 2
    )
    conv_plot(sweep, rescaled_neff2)
    save_fig(
        path,
        f"logreject-logD_byN_{method}_{param_idx}_{job_id}_rescaled_neff2",
        suffix="pdf",
        show=True,
        dpi=600,
        overwrite=True,
