...
from gpsampler.samplers import sample_rff_from_x, zrf, estimate_se_rff_kernel, estimate_mat_rff_kernel
...
class TestRFF:
    ...
    def test_rff(self, X, K, benchmarks):
        ...
        with patch('gpsampler.samplers.zrf') as mock_zrf:
    ...
    def test_se_Krff(self, X, K):
        Krff = estimate_se_rff_kernel(X, D, ks, ls)
        # Krffe = Krff + nv * np.eye(n)
        err = np.linalg.norm(K-Krff)
        assert err < 1.0
